...
var ytdl = module.exports = function(videoUrl, args, options) {
    if(typeof options.bin === 'string'){
        ytdl.setBinPath(options.bin);
    }else{
        var detailsPath = path.join(__dirname, '..', 'bin/details');

        if (fs.existsSync(detailsPath)) {
          var details = JSON.parse(fs.readFileSync(detailsPath));
          ytdl.ytdlBinary = (details.path) ? details.path : path.resolve(__dirname, '..', 'bin', details.exec);
        }
    }
    
    // Check that youtube-dl file exists.
    if (!fs.existsSync(ytdl.ytdlBinary)) {
      console.log('ERROR: unable to locate youtube-dl details in ' + path.dirname(ytdl.ytdlBinary));
      process.exit(1);
    }
...
function call(urls, args1, args2, options, callback) {
  ...
  // Call youtube-dl.
  execFile(ytdl.ytdlBinary, args, options, function done(err, stdout, stderr) {
    if (err) {
